...
So, to deploy Greenlight on a relative URL root path of **/gl** one would simply run `GL_PATH=/gl bbb-install-2.6 [options] -g`.

Alternatively one could directly update the Greenlight .env file located at `/root/greenlight-v3/.env` and manually set the variable **RELATIVE_URL_ROOT** to match their desired setup and simply re-run afterwards the bbb-install command **with the -g option included** but without using the **GL_PATH**.

_We recommend the use of the first approach whenever possible._

> The **GL_PATH** if defined on the shell session will always be prioritized over the **RELATIVE_URL_ROOT** (even if the latter is set).
